...
class Users(db.Model, UserMixin):
    ...
    @classmethod
    def create_user(cls, username: str, password: str, pin_code: str) -> None:
        salt, hashed_password = cls._generate_hashed_password(password)
        new_user = cls(username=username, salt=salt, password=hashed_password, pinCode=pin_code)
        try:
    ...
    def update_pincode(cls, username: str, pincode: str) -> None:
        ...
        db.session.commit()
        logger.info(f"Pincode successfully updated to {pincode} for user: %s", username)
